...
const FoodEdit = () => {
  ...
  const [coverImage, setCoverImage] = useState(recipe?.cover_image ||""); // Cover image URL
    ...
    const [splitTagList, setSplitTagList] = useState([]); // To hold the split tag list
    const handleClose = () => {
    ...
    useEffect(() => {
      // Ensure we have valid tags data (an array of strings)
      if (recipe?.tags && Array.isArray(recipe.tags)) {
        const allTags = recipe.tags.flatMap(tag => tag.split(',').map(t => t.trim())); // Split and trim all tags
        setTags(allTags);
      } else {
        setTags([]); // Empty array if no tags exist
      }
    }, [recipe?.tags]);
  ...
  useEffect(() => {
    ...
    console.log("Stored Recipe:", storedRecipe); // Log this to check the stored data
    if (storedRecipe) {
      const parsedData = JSON.parse(storedRecipe);
      console.log("Parsed Data:", parsedData); // Log the parsed data
      setRecipe(parsedData.recipe); // Set state with the inner recipe object
      console.log("Parsed Data recipe:", parsedData.recipe);
    }
  }, []);
...
const splitTags = (tags) => {
  ...
  return tags[0].split(',').map((tag) => tag.trim());
};

  // Handle adding new tag
  const handleAddTag = () => {
    const sanitizeText = (text) => text.replace(/[^a-zA-Z0-9ก-๙\s]/g, "").trim();
    ...
    const handleDeleteTags = (index) => {
      if (tags[index] !== "AI generate") {
        ...
        setTags(updatedTags); // Correctly update the state with the new list
      } else {
...
const handleAddIngredient = () => {
  if (newIngredient.trim()) {
    setIngredients((prevIngredients) => {
      const updatedIngredients = [...prevIngredients, newIngredient.trim()];
...
const handleDelete = (type, index) => {
  if (type === "ingredient") {
    ...
    if (tags[index] === "AI generate") {
      ...
      // Confirmation for deleting a tag
    ...
    <Box sx={{ padding: "2rem", backgroundColor: "#f9f9f9", minHeight: "100vh" }}>
      ...
      <Box sx={{ marginBottom: "2rem" }}>
        <Typography variant="h6" sx={{ fontWeight: "bold" }}>
          หมวดหมู่ (Category): {recipe?.category}
        </Typography>
      </Box>
      ...
      >
        {/* Ingredients and Instructions in a Grid */}
          ...
          <Button
            ...
            onClick={async () => {
              ...
              try {
                ...
                const checkResponse = await fetch(`${process.env.NEXT_PUBLIC_BACK_END_URL}/gemini/check/recipe/edit`, {
                  method: "POST",
                ...
                if (checkData.status === "Safe") {
                  const submitResponse = await fetch(`${process.env.NEXT_PUBLIC_BACK_END_URL}/api/recipes/edit`, {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
                    credentials: "include",
                    body: JSON.stringify(recipeData),
